...
for (type in types) {
  ...
  n <- lapply(conds, function(cond){
    ...
    net <- read_tsv(paste0(type, "/networks/1e8/", type, "-", cond, "-min.sif"))
    ...
    write.table(vertices, file = paste0(type, "/networks/network-tables/",
                                             type,"-", cond, "-vertices.tsv") , 
                quote = F, row.names = F, col.names = T, sep = "\t")
    write.table(net, file = paste0(type, "/networks/network-tables/", 
                                        type ,"-", cond, "-interactions.tsv"),
                quote = F, row.names = F, col.names = T, sep = "\t")
  })
}
